...
-- Mon 24 Mar 2025 10:38:40 AM CET
-- Model: Univ6    Version: 2.4
...
-- -----------------------------------------------------
-- procedure POrdersGetWithCustomer
-- -----------------------------------------------------

DELIMITER $$
USE `pear`$$
CREATE PROCEDURE POrdersGetWithCustomer(
	IN p_OrderID INT,
...
BEGIN
    ...
    FROM `Order` o
    INNER JOIN Customer c ON o.Customer_CustomerKey = c.CustomerKey
    LEFT JOIN OrderDetail od ON o.OrderKey = od.Order_OrderKey
    LEFT JOIN Product p ON od.Product_ProductKey = p.ProductKey
    WHERE o.OrderKey = p_OrderID AND o.SalesPoint_SalesPointKey = p_SalesPointKey;
END$$
...
BEGIN
    INSERT INTO `Order` (OrderTime, DeliveryDate, DeliveryCost, OrderStatus, SalesPoint_SalesPointKey, Customer_CustomerKey)
    VALUES (p_OrderTime, p_DeliveryDate, p_DeliveryCost, p_OrderStatus, p_SalesPointKey, p_CustomerKey);
END$$
